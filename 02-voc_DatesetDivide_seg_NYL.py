...
np.random.seed(2)  # 定义随机数种子 #2 分割 

xmlfilepath=r'./SegmentationClass_Pseudo_1370/'     # 所有标注文件
...
def voc2frcnn(train_percent = 0.7,val_percent = 0.15, test_percent = 0.15):  #设置函数训练、验证、测试数据集比例
    ...
    prename = 'origin'
    ftest = open(os.path.join(saveBasePath, prename + 'test.txt'), 'w')  
    ftrain = open(os.path.join(saveBasePath, prename + 'train.txt'), 'w')  
    fval = open(os.path.join(saveBasePath, prename + 'val.txt'), 'w')  
...
if __name__ == "__main__": 
    voc2frcnn(train_percent = 0.80,val_percent = 0.2, test_percent = 0.0) #设置函数训练、验证、测试数据集比例
